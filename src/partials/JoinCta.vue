<template>
  <section class="bg-linear-to-br from-white to-indigo-50/20">
    <div class="py-12 md:py-20">
      <div class="max-w-6xl mx-auto px-4 sm:px-6">
        <div class="relative max-w-3xl mx-auto text-center pb-12 md:pb-16">
          <div class="inline-flex items-center justify-center w-20 h-20 bg-white rounded-xl shadow-md mb-8 relative before:absolute before:-top-12 before:w-52 before:h-52 before:bg-zinc-900 before:opacity-[.08] before:rounded-full before:blur-3xl before:-z-10">
            <img src="../images/logo-02.png" width="60" height="60" alt="Logo" />
          </div>
          <h2 class="font-inter-tight text-3xl md:text-4xl font-bold text-zinc-900 mb-4">
            Envie de rejoindre un collectif
            <em class="relative not-italic inline-flex justify-center items-end">
              d’architectes exigeants ?
              <svg class="absolute fill-zinc-300 w-[calc(50%+1rem)] md:w-[calc(100%+1rem)] -z-10" xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 120 10" aria-hidden="true" preserveAspectRatio="none">
                <path d="M118.273 6.09C79.243 4.558 40.297 5.459 1.305 9.034c-1.507.13-1.742-1.521-.199-1.81C39.81-.228 79.647-1.568 118.443 4.2c1.63.233 1.377 1.943-.17 1.89Z" />
              </svg>
            </em>
          </h2>
          <p class="text-lg text-zinc-500 mb-8">
            Partagez-nous votre parcours, vos expertises et les sujets sur lesquels vous souhaitez intervenir. Nous reviendrons vers vous pour un premier échange.
          </p>
          <div class="max-w-xs mx-auto sm:max-w-none sm:inline-flex sm:justify-center space-y-4 sm:space-y-0 sm:space-x-4">
            <div>
              <router-link class="btn text-zinc-100 bg-zinc-900 hover:bg-zinc-800 w-full shadow-sm" to="/c-02-f">Rejoindre la communauté</router-link>
            </div>
            <div>
              <router-link class="btn text-zinc-600 bg-white hover:text-zinc-900 w-full shadow-sm" to="/nous-contacter">Échanger avec Wenovat</router-link>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>
</template>

<script setup lang="ts">
</script>
